...
class Web_App_Model extends CI_Model {
	...
	public function getLoginData($usr,$psw)
	{
		...
		{
			return $this->db->query("SELECT a.nim, a.nama_mahasiswa, a.angkatan, a.jurusan, a.kelas_program, b.kd_dosen FROM tbl_mahasiswa a left join tbl_dosen_wali b on a.nim=b.nim where a.nim='".$nim."'");	
		}
	...
	function getDosenMK($kd)
	{
		return $this->db->query("SELECT a.kd_dosen, a.nama_dosen, b.nama_mk, b.kd_mk,b.jadwal FROM tbl_dosen a left join 
								(select y.kd_mk,x.kd_dosen, y.nama_mk,x.jadwal from tbl_jadwal x left join tbl_mk y on x.kd_mk=y.kd_mk) b on a.kd_dosen=b.kd_dosen where a.kd_dosen='".$kd."' and b.kd_mk!=''");	
	}
	...
	//query untuk mengambil daftar dosen yang diajarmatakuliah
	function getMkDosen($kd)
	{
		return $this->db->query("SELECT a.kd_mk, a.nama_mk, b.nama_dosen, b.kd_dosen,b.jadwal FROM tbl_mk a left join 
								(select x.kd_mk,y.kd_dosen, y.nama_dosen,x.jadwal from tbl_jadwal x left join tbl_dosen y on x.kd_dosen=y.kd_dosen) b on a.kd_mk=b.kd_mk where a.kd_mk='".$kd."' and b.kd_dosen!=''");	
	}
	...
	{
		$q = $this->db->query("SELECT * FROM tbl_perwalian_header a left join tbl_mahasiswa b on a.nim=b.nim left join tbl_dosen_wali c on a.nim=c.nim where a.status='1' LIMIT ".$offset.",".$limit."");
		return $q;
	}
	...
	{
		$q = $this->db->query("SELECT a.nim, b.nama_mahasiswa, c.j_sks, b.jurusan, b.kelas_program, e.status from tbl_dosen_wali a left join tbl_mahasiswa b on a.nim=b.nim left join (select k.nim, k.kd_jadwal, SUM(l.jum_sks) as j_sks from tbl_perwalian_detail k left join (select x.kd_jadwal, y.jum_sks from tbl_jadwal x left join tbl_mk y on x.kd_mk=y.kd_mk) as l on k.kd_jadwal=l.kd_jadwal) c on a.nim=c.nim left join tbl_perwalian_header e on a.nim=e.nim where a.kd_dosen='".$kd_dosen."' and e.status=1 group by a.nim");
		return $q;
	}
	...
	//query jadwal
	function getJadwal($nim,$kelas_program)
	{
		return $this->db->query('SELECT 
		a.kd_jadwal,b.nama_mk,b.kd_mk,b.semester,b.jum_sks,a.kapasitas,a.kelas,c.kd_dosen,a.jadwal,d.Peserta,d.CalonPeserta,c.nama_dosen FROM 
		`tbl_jadwal` a 
		...
		on a.kd_jadwal=d.kd_jadwal where a.kd_mk not in (select kd_mk from tbl_nilai where nim="'.$nim.'" AND grade NOT IN("E","T")) and a.kelas_program="'.$kelas_program.'"');	
	}
